...
002_CIO_pipeline_SCTransform was adapted to create this basic **murine** data pipeline for multiple samples allowing a user with basic R skills to analyze 10X scRNAseq using Seurat. Required input files are cellranger output files (filtered_counts.h5), as will typically be received from a sequencing facility (SCAF). **THIS IS A TRIAL-RUN WITH IN-HOUSE MURINE TUMOR DATASET**. This setup is heavily based upon the Seurat tutorial, available at https://satijalab.org/seurat/articles/pbmc3k_tutorial and adapted with NIDAP-style QC and visuals.
...
setwd("/gpfs/gsfs12/users/minnarcm/R/Project_Template1") ## set this to your data dir; see container directions
```
...
## Sample 1
T1 <- Read10X_h5("Trial_dataset/SCAF4396_T1_GEX_filtered_feature_bc_matrix.h5") 
...
## Sample 2
T2 <- Read10X_h5("Trial_dataset/SCAF4397_T2_GEX_filtered_feature_bc_matrix.h5") 
...
##Sample 3
T3 <- Read10X_h5("Trial_dataset/SCAF4398_T3_GEX_filtered_feature_bc_matrix.h5") 
...
##Sample 4
T4 <- Read10X_h5("Trial_dataset/SCAF4399_T4_GEX_filtered_feature_bc_matrix.h5") 
...
#### View column labels
```{r}
# Show QC metrics for the first 5 cells: allows you to see the metadata you've added per sample
head(obj1@meta.data, 5)
head(obj2@meta.data, 5)
head(obj3@meta.data, 5)
head(obj4@meta.data, 5)
...
## FILTER SAMPLES (EDIT AS NEEDED HERE AFTER VIEWING VIOLIN PLOTS)
# nCount_RNA
ncounts.low         = NA
ncounts.high        = NA
mad.ncounts.low     = 3
...
filtered_objs <- lapply(objs, function(obj) {
 ...
 # 2) This steip keeps your cells that meet QC thresholds
...
# 4) You are creating a list of all of your filtered samples
filtered_objs <- lapply(filtered_objs, function(obj_qc) {
  obj.sce <- as.SingleCellExperiment(obj_qc)
  bp <- MulticoreParam(3, RNGseed=1234)
  obj.sce <- scDblFinder(obj.sce, BPPARAM=bp)

  obj_qc$scDblFinder.score <- obj.sce$scDblFinder.score
  obj_qc$scDblFinder.class <- obj.sce$scDblFinder.class
 
  rm(obj.sce)
  
   # >>> view singlets vs doublets BEFORE removal <<<
  print(table(factor(obj_qc$scDblFinder.class, levels = c("singlet","doublet")),
              useNA = "ifany"))
  
# 5) Drop doublets and apply filtered list to SOs
  obj_qc[, obj_qc$scDblFinder.class != "doublet", drop = FALSE]
})

sapply(filtered_objs, function(o)
+     table(FetchData(o, "scDblFinder.class")[,1], useNA = "ifany"))
...
rm(obj1, obj2, obj3, obj4, objs, filtered_objs, df, df_list, df_long, df_long2, filtered_df, filtered_df_list, filtered_long, filtered_QC_violin)
gc()
```
...
merged <- RunUMAP(merged, reduction = "pca", dims = 1:30, seed.use = 42)
#merged <- RunTSNE(merged, reduction = "pca", dim.embed = 2, dims = 1:30, seed.use = 1) # optional if you want to see TSNE
```
...
Cluster_genes <- VlnPlot(merged,
        features=c('Cd3e', 'Cd8a', 'Cd8b1', 'Cd4', 'Foxp3', 'Cd19', 'Ncr1', 'Nos2', 'Adgre1', 'Mrc1', 'Clec9a'),
        group.by='SCT_snn_res.0.4',
        cols=cbf_3,
        stack=TRUE,
        flip=TRUE)
...
```{r}
#| label: feature-plots-known-marker-genes
...
SaveSeuratRds(merged, file='Output/mergedSO010926.rds')
```